...
export default function GenrePage() {
  ...
  useEffect(() => {
    ...
    getTempData()
      .then((jsonData) => {
        ...
        setTotalPages(
          Math.min(jsonData.total_pages || 1, 500)
        );
  ...
  const handleGenreClick = (genreId) => {
    ...
    if (selectedGenreIds.includes(idStr)) {
      updatedIds = selectedGenreIds.filter(
        (id) => id !== idStr
      );
    } else {
  ...
  const currentGenreNames =
    data
      ?.filter((g) =>
        selectedGenreIds.includes(String(g.id))
      )
      ?.map((g) => g.name)
      ?.join(", ") || "All";
