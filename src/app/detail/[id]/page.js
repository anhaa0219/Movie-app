...
export default function Detail() {
  ...
  const fetchTMDB = async (endpoint) => {
    ...
    if (!response.ok) {
      console.error("TMDB ERROR:", endpoint, response.status, jsonData);
  ...
  if (errorMessage || !data) {
    return (
      <div className="min-h-screen bg-white dark:bg-zinc-950 p-8 md:p-12 text-center text-red-500 flex flex-col items-center justify-center gap-3">
        ...
        <button
          ...
          className="px-4 py-2 text-sm bg-zinc-800 dark:bg-zinc-700 text-white rounded-md hover:bg-zinc-700 dark:hover:bg-zinc-600 transition-colors"
        >
          Try Again
        </button>
      </div>
